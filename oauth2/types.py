...
class Client(collections.namedtuple(
    ...
    - identifier (:rfc:`6749#section-2.2`): unique string. Using a uuid
      version 4 suffices, I think. URL safety would be nice. For example::
    ...
    -   secret / password (:rfc:`6749#section-2.3.1`):
        -   only used by trusted clients
        -   random string with at least 128 bits of entropy. For base 64 encoded
            secrets that means 6 bits per byte / character, so ~22 characters.
